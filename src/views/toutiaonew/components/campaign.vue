            ...
            <Form :label-width="100">
              <FormItem label="选择推广目的">
                ...
                <RadioGroup v-model="landing_type" type="button" size="large">
                  <Radio :disabled="id && item.val_type !== landing_type" :label="item.val_type" v-for="item in toutiaoConfig.landing_type" :key="item">{{item.name}}</Radio>
                </RadioGroup>
              </FormItem>
              <FormItem label="预算">
                <span class="required-item"></span>
                <RadioGroup v-model="budget_mode" type="button" size="large">
                  <Radio :label="item.val_type" v-for="item in toutiaoConfig.budget_mode" :key="item" :disabled="item.val_type === 'BUDGET_MODE_TOTAL'">{{item.name}}</Radio>
                </RadioGroup>
              </FormItem>
